...
def main(urls, timeout, verbose):
    ...
    if verbose:
        logger.debug(f"Received Verbose mode is on {verbose}. Setting log level to DEBUG.")
    ...
    if not urls:
        ...
        click.echo("usage: check-urls <URL> [<URL> ...] [--timeout <seconds>] [--verbose]")
        return
    else:
        logger.debug(f"Received {urls} URLs to check.")
    
    logger.info(f"Starting HTTP checker for {len(urls)} URLs with a timeout of {timeout} seconds.")
