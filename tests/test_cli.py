...
def test_main_single_url(mocker: MockerFixture):
    ...
    mock_check_urls = mocker.patch("simple_http_checker.cli.check_urls", return_value={url: "200 OK"})
    runner = CliRunner()
...
def test_main_timeout_option(mocker: MockerFixture):
    url = "https://example.com"
    timeout = 10
    mock_check_urls = mocker.patch("simple_http_checker.cli.check_urls", return_value={url: "TIMEOUT"})
    runner = CliRunner()
